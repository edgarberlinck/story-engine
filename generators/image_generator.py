...
def setup_model_directories():
    ...
    model_dirs = ["models/diffusion", "models/segmentation", "models/text_generation"]
...
def generate_images(
    ...
    try:
        ...
        if model_name == "sdxl":
            ...
            pipe = FluxPipeline.from_pretrained(model_path, torch_dtype=torch_dtype)
        ...
        filename_base = (
            task_name if task_name else generate_filename_from_prompt(prompt)
        )
...
def benchmark_models(*args, **kwargs):
    ...
    print(
        "Warning: benchmark_models moved to generators.benchmark_image_generator; "
...
def main():
    ...
    parser.add_argument(
        "--model", default=None, help="Diffusion model to use (default: sdxl)"
    )
    parser.add_argument(
        "--amount",
        type=int,
