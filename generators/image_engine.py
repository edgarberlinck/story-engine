...
def generate(
    ...
    **kwargs: Any,
    ...
    return generate_images(
        prompt=prompt, model_name=model_name, model_path=model_path, **image_kwargs
    )
...
def get_character(
    name: str, project: str = DEFAULT_PROJECT
...
def _enrich_scene_prompt(
    prompt: str, project: str, use_appearance_only: bool = False
) -> str:
    ...
    if len(characters) >= 1:
        ...
        # Log token usage
        print(
            f"Token budget: {stats['total_tokens_estimated']}/{stats['max_tokens']} tokens"
        )
        if stats["items_dropped"] > 0:
            print(
                f"Warning: Dropped {stats['items_dropped']} items due to token budget"
            )
            for dropped in stats["dropped_details"]:
                print(
                    f"  - Dropped {dropped['category']} (priority {dropped['priority']}): {dropped['text']}"
                )
...
def generate_scene(
    ...
    **kwargs: Any,
    ...
    if use_asset_pipeline and len(characters) >= 2:
        ...
        print(
            f"INFO: {len(characters)} characters detected, delegating to asset-composition pipeline"
        )
    ...
    if use_style_mediation and len(characters) >= 2:
        ...
        if use_appearance_only and style_warnings:
            print(
                f"INFO: Using appearance-only mode for scene to avoid style conflicts"
            )
    ...
    if use_advanced_prompting and len(characters) >= 2:
        ...
        if style_ids:
            ...
            if recommended_model != model:
                print(
                    f"INFO: Recommended model for style combination: {recommended_model} "
    ...
    if enforce_token_budget and len(characters) >= 1:
        ...
        print(
            f"Token analysis: Original={original_token_count}, Token-aware={new_token_count}"
        )

        if (
            new_token_count < original_token_count
            or stats["total_tokens_estimated"] > 70
        ):
            print(
                f"INFO: Using token-aware prompt (saved {original_token_count - new_token_count} tokens)"
            )
