...
class AnimateSceneEnhanceTest(unittest.TestCase):
    def _tiny_video(self, tmp, name="raw"):
        ...
        return video_enhancer.write_frames(
            os.path.join(tmp, name), frames, fps=5.0, container="mp4", crf=18
        )
    ...
    def test_enhance_true_adds_enhanced_keys(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                video_engine, "scene_out_dir", return_value=out_dir
            ), patch.object(
                video_engine, "generate_video", return_value=self._raw_result(raw)
            ) as gen:
    ...
    def test_enhance_false_does_not_add_enhancement_keys(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            raw = self._tiny_video(tmp)
            with patch.object(
                video_engine, "scene_out_dir", return_value=Path(tmp)
            ), patch.object(
                video_engine, "generate_video", return_value=self._raw_result(raw)
            ) as gen:
    ...
    def test_enhancement_failure_degrades_to_raw_video(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                ...
                video_engine, "generate_video", return_value=self._raw_result(missing)
            ), patch(
                "builtins.print"
            ):
