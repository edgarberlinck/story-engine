...
class VideoEngineCoverageTest(unittest.TestCase):
    ...
    def test_create_validated_scene_combines_single_and_list_names(self):
        ...
        with patch.object(
            ve, "get_character", side_effect=lambda name, project: chars[name]
        ) as getc, patch.object(ve, "generate_scene", return_value=scene), patch.object(
            ve, "verify_character_in_scene", return_value=True
        ) as verify:
            out = ve.create_validated_scene(
                "p", character_name="A", character_names=["B"], project="proj"
            )
    ...
    def test_create_validated_scene_false_when_not_required_after_attempts(self):
        ...
        scene = {"scene_number": 1, "image_path": "scene.png", "prompt": "p"}
        with patch.object(ve, "get_character", return_value=char), patch.object(
            ve, "generate_scene", return_value=scene
        ) as gen, patch.object(ve, "verify_character_in_scene", return_value=False):
            out = ve.create_validated_scene(
                "p", character_name="A", max_attempts=2, require_verification=False
            )
    ...
    def test_animate_scene_uses_enriched_prompt_and_scene_out_dir(self):
        scene = {
            ...
            "prompt": "raw",
            "enriched_prompt": "rich",
        }
        with patch.object(
            ve, "scene_out_dir", return_value="/tmp/out"
        ) as out_dir, patch.object(
            ve, "generate_video", return_value={"video_path": "v.mp4"}
        ) as gen:
            result = ve.animate_scene(
                scene, model_name="model", project="proj", seed=99, fps=5
            )
