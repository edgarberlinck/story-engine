...
Notes on running Wan 2.2 I2V A14B on this machine.
...
- `generators/video_generator.py` targets `wan22_i2v` via diffusers

## Verdict

The constraint is throughput, not memory.

## Wan 2.2 I2V A14B — runs, but only if quantized
...
1. **Skip torch-MPS for Wan 2.2.** Diffusers' Wan2.2 pipeline on Metal is
   ...
   dramatically faster.
2. Thermal note: sustained 14B inference will throttle the M5 Pro over long
   runs, but a 40-step @ Q4 generation finishes before that becomes a real
   problem.
3. Memory is not an issue — Wan 2.2 can run comfortably with quantization.
