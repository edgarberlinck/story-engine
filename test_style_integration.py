    ...
    for i, case in enumerate(test_cases, 1):
        ...
        try:
            ...
            char_type = (
                "man"
                if case["gender"] == "male"
                else "woman" if case["gender"] == "female" else "animal"
            )

            # Build minimal attributes for prompt function (style needs to be in the attributes dict)
            attrs_for_prompt = full_attrs.copy()
            attrs_for_prompt["character_type"] = (
                char_type  # This should be removed, let's test with proper structure
            )
            ...
            prompt = build_character_prompt(
                ...
                attributes={
                    k: v for k, v in full_attrs.items() if k not in ["gender", "name"]
                },
            )
