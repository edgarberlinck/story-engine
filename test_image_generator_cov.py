...
class _FakeMetrics:
    ...
    def record_generation(self, *args): self.recorded = args
    def save_metrics(self, base, model): return f"{base}_{model}.json"
...
class ImageGeneratorCoverageTest(unittest.TestCase):
    ...
    def test_resolve_model_path_prefers_local_nonempty(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(ig, "project_root", tmp), patch.object(ig, "MODEL_PATHS", {"diffusion": "models/diffusion"}):
                local = os.path.join(tmp, "models/diffusion/sdxl")
                os.makedirs(local)
                self.assertEqual(ig.resolve_model_path("diffusion", "sdxl", "hub"), "hub")
                open(os.path.join(local, "file"), "w").close()
                self.assertEqual(ig.resolve_model_path("diffusion", "sdxl", "hub"), local)

    def test_cleanup_pipeline_clears_cuda_and_mps_branches(self):
        with patch.object(ig.gc, "collect") as collect, \
             patch.object(ig.torch.backends.mps, "is_available", return_value=True), \
             patch.object(ig.torch.mps, "empty_cache") as mps_empty:
            ig.cleanup_pipeline(object())
        collect.assert_called_once(); mps_empty.assert_called_once()

        with patch.object(ig.gc, "collect"), \
             patch.object(ig.torch.backends.mps, "is_available", return_value=False), \
             patch.object(ig.torch.cuda, "is_available", return_value=True), \
             patch.object(ig.torch.cuda, "empty_cache") as cuda_empty:
    ...
    def test_generate_images_sdxl_and_flux_dev_call_expected_kwargs(self):
        ...
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch.object(ig.StableDiffusionXLPipeline, "from_pretrained", return_value=sdxl_pipe):
            out = ig.generate_images("blue bird", num_images=2, model_name="sdxl", task_name="task")
        self.assertEqual(out, ["outputs/task_sdxl_benchmark.png", "outputs/task_sdxl_benchmark.png"])
        self.assertEqual(sdxl_pipe.calls[0][1]["num_images"], 2)

        flux_pipe = _FakePipe(_FakeResult(1))
        patches = self._patch_common()
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch.object(ig.FluxPipeline, "from_pretrained", return_value=flux_pipe):
    ...
    def test_generate_images_invalid_and_exception_cleanup(self):
        ...
        patches = self._patch_common()
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch.object(ig.FluxPipeline, "from_pretrained", return_value=bad_pipe):
            with self.assertRaises(RuntimeError):
                ig.generate_images("p", model_name="flux_dev")

    def test_generate_image_success_multiple_default_and_fallback_generation_failure(self):
        pipe = _FakePipe(_FakeResult(1))
        with patch.object(ig, "get_model_config", return_value=("cpu", "float16")), \
             patch.object(ig.StableDiffusionXLPipeline, "from_pretrained", return_value=pipe), \
             patch.object(ig, "cleanup_pipeline"):
            out = ig.generate_image("hello world", amount=2, output_filename="bad name!")
        self.assertEqual(out, ["outputs/badname_001.png", "outputs/badname_002.png"])
        self.assertEqual(pipe.calls[0][0][0], "hello world")

        first_loader = MagicMock(side_effect=[RuntimeError("primary failed"), _FakePipe(exc=RuntimeError("gen fail"))])
        with patch.object(ig, "get_model_config", return_value=("cpu", "float16")), \
             patch.object(ig.StableDiffusionXLPipeline, "from_pretrained", first_loader), \
             patch.object(ig, "cleanup_pipeline"), \
    ...
    def test_generate_image_fallback_failure_raises_value_error(self):
        with patch.object(ig, "get_model_config", return_value=("cpu", "float16")), \
             patch.object(ig.StableDiffusionXLPipeline, "from_pretrained", side_effect=RuntimeError("load failed")):
            with self.assertRaises(ValueError):
                ig.generate_image("p")
