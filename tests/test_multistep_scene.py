...
from core.scene_composer import (
    MultiStepSceneGenerator,
    SceneComposer,
...
def _plan(strategy="asset_composition", canvas_layout=None):
    resolved = [
        ResolvedCharacter(
            ...
            [],
            "",
            scene_position_hint="left",
        ),
        ResolvedCharacter(
            "Roger", ["man"], [], "KEEP", ["shirt"], [], "", scene_position_hint="right"
        ),
    ]
    return ScenePlan(
        camera="wide",
        layers=[
            SceneLayerPlan("base_environment", "empty bar stage", ["stage"], "full")
        ],
...
class TestSceneComposer(unittest.TestCase):
    def test_scene_decomposition_creates_base_and_character_layers(self):
        ...
        self.assertTrue(
            any("playing a black Gibson" in layer.prompt for layer in plan.layers)
        )
    ...
    def test_decomposition_uses_default_environment_when_no_env_lines(self):
        plan = SceneComposer.decompose_scene(
            "Nikita smiles.", [{"name": "Nikita", "prompt": "woman"}]
        )
    ...
    def test_incremental_prompts_handle_blank_camera_setup(self):
        plan = SceneCompositionPlan(
            "empty room", [], "", [SceneLayer("base_environment", "empty room")]
        )
        self.assertEqual(
            SceneComposer.build_incremental_prompts(plan), [("base", "empty room")]
        )
...
class TestMultiStepSceneGenerator(unittest.TestCase):
    def test_generate_scene_incrementally_mocks_image_generation_and_copies_steps(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch(
                "generators.image_engine.generate_images",
                side_effect=fake_generate_images,
            ), patch(
                "utils.project_paths.scene_dir", return_value=temp_root / "scene_001"
            ):
...
class TestScenePipelineHelpers(unittest.TestCase):
    def test_select_strategy_and_background_prompt_variants(self):
        ...
        plan = _plan()
        self.assertEqual(
            sp._build_background_prompt("Nikita plays", plan, ["Nikita"]),
            "Nikita plays. The scene is empty, no characters visible yet.",
        )
        plan.layers.append(SceneLayerPlan("character_Nikita", "Nikita", []))
        self.assertEqual(
            sp._build_background_prompt("raw", plan, ["Nikita"]), "empty bar stage"
        )
        stripped = sp._build_background_prompt(
            "A smoky bar. Nikita plays. She smiles. The crowd cheers.",
            ScenePlan("", [], True, ""),
            ["Nikita"],
        )
    ...
    def test_reference_prompt_and_refinement_prompt_and_canvas_layout(self):
        plan = _plan(
            canvas_layout={
                ...
                "height": 512,
                "placements": [
                    {"name": "Nikita", "anchor": [0.2, 0.8], "scale": 0.4, "z": 2}
                ],
            }
        )
        prompt = sp._build_reference_conditioned_prompt(
            "A stage.", plan, [{"name": "Nikita", "path": "n.png"}]
        )
        ...
        refinement = sp._build_refinement_prompt(
            "photorealistic wide cinematic shot with natural stage lighting"
        )
    ...
    def test_qa_scene_success_missing_reference_and_exception(self):
        ...
        with patch(
            "generators.image_engine.get_character",
            side_effect=lambda name, project: records.get(name),
        ), patch("utils.face_check.character_appears_in_image", side_effect=fake_face):
            qa = sp._qa_scene(
                "final.png", ["Nikita", "Roger", "Missing", "Bad"], "Project"
            )
    ...
    def test_qa_scene_marks_false_as_failed(self):
        with patch(
            "generators.image_engine.get_character",
            return_value={"reference_image": "ref.png"},
...
class TestScenePipelineAssetComposition(unittest.TestCase):
    def test_asset_composition_refinement_disabled_uses_composite_and_region_hints(
        self,
    ):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            plan = ScenePlan(
                ...
                "r",
                resolved_characters=[
                    ResolvedCharacter("Nikita", ["woman"], [], "KEEP", [], [], "")
                ],
            ...
            def fake_compose(
                background_path, placements, canvas_width, canvas_height, output_path
            ):
            ...
            with patch(
                "core.scene_pipeline.scene_dir", return_value=root / "scene"
            ), patch(
                "generators.image_engine.generate_images", return_value=[source_bg]
            ) as mock_bg, patch(
                "core.scene_pipeline.generate_character_assets",
                return_value=[CharacterAsset("Nikita", source_asset, "prompt", "ref")],
            ), patch(
                "core.scene_compositor.segment_character",
                return_value=("mask.png", cutout, (0, 0, 10, 10), "mock"),
            ), patch(
                "core.scene_compositor.validate_character_asset",
                return_value={
                    "valid": True,
                    "issues": [],
                    "metrics": {},
                ...
                "core.scene_pipeline.compose_scene", side_effect=fake_compose
            ), patch(
                "core.scene_pipeline._qa_scene",
                return_value={"per_character": {"Nikita": True}, "passed": True},
            ):
                result = sp._run_asset_composition(
                    "Nikita plays on a stage",
                    "Project",
                    7,
                    plan,
                    "sdxl",
    ...
    def test_asset_composition_invalid_segmentation_retries_and_warns_then_refinement_fails(
        self,
    ):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            with patch(
                ...
                "core.scene_pipeline.generate_character_assets",
                return_value=[CharacterAsset("Roger", source_asset, "prompt", None)],
            ), patch(
                "core.scene_compositor.segment_character",
                side_effect=[
                    ("m", "bad_cutout.png", (1, 2, 3, 4), "simple"),
                    ("m2", None, None, "aggressive"),
                ],
            ) as mock_segment, patch(
                "core.scene_compositor.validate_character_asset", side_effect=checks
            ), patch(
                "core.scene_pipeline.compose_scene", side_effect=fake_compose
            ), patch(
                "generators.img2img_engine.refine_composite",
                side_effect=RuntimeError("refine no"),
            ), patch(
                "core.scene_pipeline._qa_scene",
                return_value={"per_character": {"Roger": False}, "passed": False},
            ):
                result = sp._run_asset_composition(
                    "A bar", "Project", 8, _plan(), "sdxl", 1, enable_refinement=True
                )
...
class TestGenerateScenePipelineFallbacks(unittest.TestCase):
    def test_single_pass_path_without_characters_uses_generate_scene(self):
        with patch(
            "core.scene_pipeline.generate_scene",
            return_value={"image_path": "scene.png"},
        ) as mock_generate:
            result = sp.generate_scene_pipeline(
                "empty landscape",
                project="P",
                scene_number=1,
                characters=[],
    ...
    def test_reference_generation_failure_falls_back_to_progressive(self):
        ...
        with patch(
            ...
            **{
            ...
            return_value=[{"name": "Nikita", "path": "ref.png"}],
        ), patch(
            "core.scene_pipeline._run_reference_conditioned_scene",
            side_effect=RuntimeError("reference backend died"),
        ), patch(
            "core.scene_pipeline._run_progressive",
            return_value={
                "image_path": prog_path,
                "scene_number": 2,
    ...
    def test_asset_composition_failure_falls_back_to_progressive(self):
        ...
        with patch(
            ...
            **{
            ...
            "core.scene_pipeline.supports_reference_conditioning", return_value=False
        ), patch(
            "core.scene_pipeline._run_asset_composition",
            side_effect=RuntimeError("asset failed"),
        ), patch(
            "core.scene_pipeline._run_progressive",
            return_value={"image_path": prog_path, "scene_number": 3},
        ) as mock_prog:
            result = sp.generate_scene_pipeline(
                "band",
                project="P",
