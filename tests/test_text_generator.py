...
class TextGeneratorLLMTest(unittest.TestCase):
    def test_generate_prompt_with_llm_returns_first_line_and_cleans_up(self):
        ...
        with patch.object(tg, "hf_logging", SimpleNamespace(set_verbosity_error=MagicMock())), \
             patch.object(tg, "resolve_model_path", return_value="local/model"), \
             patch.object(tg, "get_model_config", return_value=("cpu", "float16")), \
             patch.object(tg, "hf_pipeline", return_value=fake) as pipeline, \
             patch("generators.image_generator.cleanup_pipeline") as cleanup:
    ...
    def test_generate_prompt_with_llm_falls_back_on_failure(self):
        with patch.object(tg, "hf_logging", SimpleNamespace(set_verbosity_error=MagicMock())), \
             patch.object(tg, "resolve_model_path", side_effect=RuntimeError("boom")), \
             patch("generators.image_generator.cleanup_pipeline") as cleanup:
    ...
    def test_generate_text_with_llm_uses_chat_template_and_temperature(self):
        ...
        with patch.object(tg, "hf_logging", SimpleNamespace(set_verbosity_error=MagicMock())), \
             patch.object(tg, "resolve_model_path", return_value="local/model"), \
             patch.object(tg, "get_model_config", return_value=("cpu", "float16")), \
             patch.object(tg, "hf_pipeline", return_value=fake), \
             patch("generators.image_generator.cleanup_pipeline"):
            out = tg.generate_text_with_llm("prompt", max_new_tokens=77, temperature=0.4)
    ...
    def test_generate_text_with_llm_returns_none_for_empty_or_exception(self):
        ...
        with patch.object(tg, "hf_logging", SimpleNamespace(set_verbosity_error=MagicMock())), \
             patch.object(tg, "resolve_model_path", side_effect=Exception("nope")), \
             patch("generators.image_generator.cleanup_pipeline"):
            self.assertIsNone(tg.generate_text_with_llm("prompt"))

    def test_resolve_model_path_prefers_nonempty_local_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(tg, "project_root", tmp), \
                 patch("models.MODEL_PATHS", {"text_generation": "models/text"}):
                local = os.path.join(tmp, "models/text/phi")
                os.makedirs(local)
                self.assertEqual(tg.resolve_model_path("text_generation", "phi", "hub"), "hub")
                with open(os.path.join(local, "config.json"), "w") as f:
                    f.write("{}")
                self.assertEqual(tg.resolve_model_path("text_generation", "phi", "hub"), local)
