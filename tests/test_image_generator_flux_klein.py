...
class FluxKleinRegistrationTest(unittest.TestCase):
    def test_flux_klein_is_registered(self):
        ...
        self.assertEqual(
            DIFFUSION_MODELS["flux_klein"], "black-forest-labs/FLUX.2-klein-4B"
        )

    def test_generate_images_flux_klein_loads_klein_not_sdxl(self):
        with patch.object(ig, "setup_model_directories"), patch.object(
            ig, "resolve_model_path", return_value="local/klein"
        ), patch.object(
            ig, "get_model_config", return_value=("cpu", "float16")
        ), patch.object(
            ig.Flux2KleinPipeline, "from_pretrained", return_value=_fake_pipe()
        ) as mock_klein, patch.object(
            ig.StableDiffusionXLPipeline, "from_pretrained"
        ) as mock_sdxl, patch.object(
            ig, "cleanup_pipeline"
        ), patch(
            "generators.image_generator.ModelMetrics", _FakeMetricTracker
        ):
            out = ig.generate_images(
                "A test scene", model_name="flux_klein", task_name="t"
            )
