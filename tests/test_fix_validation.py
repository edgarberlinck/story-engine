...
Wide shot, both characters clearly visible."""
    ...
    has_action = (
        "guitar" in prompt.lower()
        or "drums" in prompt.lower()
        or "playing" in prompt.lower()
    )
    ...
    assert (
        stats.get("base_compressed", False) or token_count < 77
    ), "Base should be compressed not dropped"
...
def test_sitting_not_stripped():
    ...
    base_prompt = (
        "Nikita is sitting on a chair playing guitar. Roger is sitting behind drums."
    )
    characters = []
    ...
    cleaned = manager.strip_generation_instructions(
...
def test_character_prompt_strips_sitting():
    ...
    cleaned = manager.strip_generation_instructions(
        char_prompt, is_character_prompt=True
    )
    ...
    assert (
        "sitting upright" not in cleaned.lower()
    ), "'sitting upright' should be stripped from char prompt"
