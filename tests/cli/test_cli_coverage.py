...
class TestCLICoverage(unittest.TestCase):
    ...
    def test_list_projects_with_data(self):
        with patch("cli.main.project_manager.list_projects",
    ...
    def test_new_project(self):
        with patch("cli.main.project_manager.create_project",
                   return_value="project_123") as mock_create:
    ...
    def test_new_project_with_description(self):
        with patch("cli.main.project_manager.create_project",
                   return_value="project_456") as mock_create:
    ...
    def test_delete_project_success(self):
        with patch("cli.main.project_manager.delete_project",
    ...
    def test_list_characters_default_project(self):
        with patch("cli.main.character_manager.list_characters",
                   return_value=[{"name": "Bob",
                                  "prompt": "a tall man standing tall"}]) as mock:
    ...
    def test_list_characters_with_option(self):
        with patch("cli.main.character_manager.list_characters",
                   return_value=[{"name": "Alice", "prompt": "a woman"}]) as mock:
            result = self.runner.invoke(app,
                                        ["list-characters", "-p", "story1"])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_generate_character_default_variants(self):
        with patch("cli.main.character_manager.generate_versions",
                   return_value=[{"version": 1}, {"version": 2},
                                  {"version": 3}]) as mock:
            result = self.runner.invoke(app,
                                        ["generate-character", "p1", "Hero", "a hero"])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_generate_character_custom_variants(self):
        with patch("cli.main.character_manager.generate_versions",
                   return_value=[{"version": 1}, {"version": 2}]) as mock:
            result = self.runner.invoke(app,
                                        ["generate-character", "p2", "Villain",
                                         "a villain", "--variants", "2"])
        self.assertEqual(result.exit_code, 0)
