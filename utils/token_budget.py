...
class TokenBudgetManager:
    ...
    def strip_generation_instructions(
        self, text: str, is_character_prompt: bool = True
    ) -> str:
    ...
    def truncate_to_tokens_smart(self, text: str, max_tokens: int) -> str:
        ...
        for idx, sent in enumerate(sentences):
            ...
            if any(
                word in sent_lower
                for word in ["playing", "sitting", "standing", "performing", "watching"]
            ):
                score += 3
            # Prefer sentences with key scene nouns
            if any(
                word in sent_lower
                for word in ["stage", "bar", "guitar", "drums", "camera"]
            ):
                score += 2
    ...
    def prioritize_character_description(
        self, character_prompt: str, attributes: Dict = None
    ) -> Tuple[str, int]:
        """Extract and prioritize character description for scenes."""
        # Strip generation instructions from CHARACTER prompts only
        cleaned = self.strip_generation_instructions(
            character_prompt, is_character_prompt=True
        )
    ...
    def build_scene_prompt(
        ...
        if base_prompt:
            ...
            base_clean = self.strip_generation_instructions(
                base_prompt, is_character_prompt=False
            )
        ...
        final_prompt = ". ".join(
            p.strip().rstrip(".") for p in final_parts if p.strip()
        )
        if final_prompt:
            final_prompt += "."
