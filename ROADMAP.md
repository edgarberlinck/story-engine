...
- [x] **Mitigated "No faces detected" hard-abort in `make benchmark-video`** (after the sentencepiece fix, the benchmark ran the full pipeline but aborted at the validated-scene stage — *not* a crash):
   ...
   - **Real root cause**: the benchmark verified against the default project (`test_project`), but Nikita & Roger live in the **`Test_ui`** project — it was checking the wrong project's data. (Initial misdiagnosis — a *broken* `dlib`/`face_recognition` embedder under Python 3.14 — was ruled out: face detection works fine on proper `Test_ui` scenes, e.g. "Nikita in scene_4/5/6: True".)
    ...
    - **Generator-vs-pipeline device mismatch, fixed (2026-08 — commit `f516543`).** `generate_video` built its RNG with `torch.Generator(device="cpu").manual_seed(seed)` even though the LTX pipeline runs on the accelerator (`enable_model_cpu_offload` → **`mps`** on Apple Silicon; `get_model_config("image_to_video")` returns `"mps"`). At sampling the pipeline samples on `mps`, so the CPU generator mismatched and the run aborted at runtime with **"Expected a 'mps' device type for generator but found 'cpu'."** **Fix:** build the generator on the already-resolved inference device — `torch.Generator(device=device)` — matching `image_generator`, `img2img_engine` and `reference_scene_generator` (all use `device=device`); correct for mps/cuda/cpu alike. **Why it stayed latent:** the previously surviving model, **Wan 2.2 A14B**, was SIGKILLed (OOM) *at `from_pretrained` load time* (see §50), so it never reached the sampling loop where the generator is used — the `device="cpu"` hardcode was never exercised. The bug only became active when the i2v model was swapped to **LTX-Video 0.9.5**, which loads and actually samples on `mps`. A regression test (`test_generate_video_uses_model_device_for_generator`) pins the generator to the model's device so a `device="cpu"` hardcode cannot silently return.
...
- [x] **Video-quality enhancement (post-generation) — *implemented* (`generators/video_enhancer.py`; see `docs/image-to-video.md` §11).** A dependency-free pass over the generated clip: temporal smoothing (kills flicker/warp) + light spatial denoise + re-encode to a high-quality low-CRF **MKV** (default CRF 12). It hooks into `animate_scene(enhance=True)` and the café benchmark and writes a `*_enhanced.mkv` + metrics next to the raw clip. The base path downloads **nothing** (numpy/scipy/imageio's bundled ffmpeg), keeping the resident i2v footprint tiny. Resolution was also pushed to **1024×576 (16:9)** with **64 steps** and the conditioning frame is now **aspect-preserving** (`video_generator._prepare_image`: clean resize when aspect matches, else contain-fit + edge replica padding — never stretched), which is what fixed the squished "aspect ratio off / movements distort the video" symptom.
- [ ] **Super-resolution up to ≥ 720p — *opt-in, not yet wired, do NOT auto-download*.** `video_enhancer.super_resolve_video` is the seam: it runs only when a local SR model (e.g. LTX latent up-sampler ~0.5 GB / Video-SR / Real-ESRGAN) is *already installed*; otherwise it is a no-op with a warning. Add an opt-in registry entry + install flag to actually enable it (this is the path past native 1024×576 toward the original ≥ 720p bar) — it must stay a separate, user-initiated step, never auto-fetched.
