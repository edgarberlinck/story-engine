...
def test_style_attribute_exists():
    ...
    animal_has_style = any(
        "Style" in category["name"] for category in animal_categories
    )
    ...
    try:
        # Test man with style
        man_attributes = {"age": "30", "ethnicity": "Caucasian", "hair_color": "Brown"}
        prompt = build_character_prompt("man", "Anime", man_attributes)
        print(f"Man prompt with style: {prompt}")

        # Test woman with style
        woman_attributes = {"age": "25", "ethnicity": "Asian", "hair_color": "Black"}
        ...
        animal_attributes = {"species": "Dog", "age": "Adult"}
