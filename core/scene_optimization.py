...
class LLMTokenOptimizer:
    ...
    def optimize_to_budget(self, spec: ScenePromptSpec, max_tokens: int = 77) -> str:
        """
...
Return only the compressed prompt, no explanations."""
    ...
    def _spec_to_json(self, spec: ScenePromptSpec) -> str:
        ...
        data = {
            "scene": {
                ...
                "camera": (
                    f"{spec.scene.shot_type} from {spec.scene.camera_position}"
                    if spec.scene
                    else ""
                ),
            },
    ...
    def _fallback_compression(self, spec: ScenePromptSpec, max_tokens: int) -> str:
        ...
        for char in spec.characters:
            ...
            if "woman" in identity_desc or "man" in identity_desc:
                ...
                for word in identity_desc.split():
                    if (
                        word in ["red", "black", "blonde", "brown"]
                        and "hair" in identity_desc
                    ):
        ...
        characters=[
            CharacterSceneData(
                ...
                identity=CharacterIdentity(
                    gender="young woman", hair_color="black", hairstyle="long"
                ),
