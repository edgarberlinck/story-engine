...
class SemanticPromptBuilder:
    ...
    def build_explicit_identity_prompt(spec: ScenePromptSpec) -> str:
        ...
        for char_data in spec.characters:
            ...
            if char_data.clothing and char_data.action:
                ...
                action_desc = (
                    f"{char_data.name}, {identity_desc}, is {char_data.action}"
                )
            else:
    ...
    def build_layered_prompts(spec: ScenePromptSpec) -> Dict[str, str]:
        ...
        appearances = []
        for char_data in spec.characters:
            appearance = SemanticPromptBuilder.build_scene_appearance_action_prompt(
                char_data
            )
    ...
    @staticmethod
    def optimize_for_token_budget(spec: ScenePromptSpec, max_tokens: int = 77) -> str:
        """
