...
class AdvancedPromptingEngine:
    ...
    @classmethod
    def build_per_character_tokens(
        cls, characters: List[Dict]
    ) -> List[CharacterStyleToken]:
        ...
        for char in characters:
            ...
            style_prefix, style_modifiers, appearance = decompose_character_prompt(
                prompt
            )
    ...
    def _build_negative_prompt(cls, style_id: Optional[str], appearance: str) -> str:
        ...
        if family == "realistic":
            ...
            negative_parts.extend(
                ["photorealistic", "anime style", "manga style", "painterly", "sketchy"]
            )
    ...
    def build_weighted_scene_prompt(
        ...
        if scene_style and scene_style in CHARACTER_STYLES:
            ...
            positive_parts.insert(
                0, f"{style_info['prefix']}, {style_info['modifiers']}"
            )
    ...
    @classmethod
    def blend_styles(
        cls, style_id_1: str, style_id_2: str, ratio: float = 0.5
    ) -> StyleBlend:
    ...
    def build_blended_prompt(
        cls, base_attributes: Dict[str, str], blend: StyleBlend
    ) -> str:
    ...
    def analyze_style_compatibility(cls, style_ids: List[str]) -> Dict:
        ...
        for i in range(len(families)):
            for j in range(i + 1, len(families)):
                ...
                weight = cls.STYLE_COMPATIBILITY_WEIGHTS.get(
                    pair
                ) or cls.STYLE_COMPATIBILITY_WEIGHTS.get(reverse_pair, 0.3)
...
def build_advanced_scene_prompt(
    ...
    positive_prompt, negative_prompt = (
        AdvancedPromptingEngine.build_weighted_scene_prompt(
