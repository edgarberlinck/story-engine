...
class SemanticSceneExtractor:
    ...
    def extract_scene_layers(
        ...
        for char_record in character_records:
            identity = SemanticSceneExtractor._extract_identity(
                char_record, scene_description
            )

            # Extract scene-specific info from description
            position, clothing, action = (
                SemanticSceneExtractor._extract_scene_attributes(
                    char_record["name"], scene_description
                )
            )
        ...
        return ScenePromptSpec(
            scene=scene_comp, characters=char_data, style=project_style
        )
    ...
    def _extract_identity(
        char_record: Dict[str, Any], scene_description: str
    ) -> CharacterIdentity:
    ...
    def _extract_scene_attributes(
        char_name: str, scene_description: str
    ) -> tuple[str, str, str]:
        ...
        sentences = [
            s.strip() for s in scene_description.split(".") if char_lower in s.lower()
        ]
        ...
        if "playing" in lower_sent:
            action = (
                sentence.split("playing")[-1].strip()
                if "playing" in sentence
                else "playing instrument"
            )
...
Return JSON only, no explanations."""
    ...
    def _parse_llm_output(self, data: Dict[str, Any], style: str) -> ScenePromptSpec:
        ...
        return ScenePromptSpec(scene=scene_comp, characters=characters, style=style)
